...
// 1. FUNCIÓN DE PARTÍCULAS MEJORADA (Lluvia de Estrellas y Emojis)
function createCelebrationParticle(emoji, type = 'float') {
    ...
    if (type === 'rain') {
        // Efecto lluvia (cae desde arriba)
        p.style.top = '-50px';
        p.style.fontSize = '20px';
        p.style.transition = 'transform 4s linear, opacity 4s linear';
        ...
        // Efecto flotar (sube desde abajo - original)
        p.style.bottom = '-50px';
        p.style.fontSize = '30px';
        p.style.transition = 'transform 3s linear, opacity 3s linear';
        document.body.appendChild(p);
        setTimeout(() => {
    ...
    setTimeout(() => p.remove(), 4100);
}

// 2. FUNCIÓN PARA DISPARAR LA LLUVIA MÁGICA
function startStarRain(duration = 5000) {
    const interval = setInterval(() => {
        createCelebrationParticle('✨', 'rain');
        createCelebrationParticle('⭐', 'rain');
    }, 200);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const bdayModalElem = document.getElementById('birthdayModal');
    const momModalElem = document.getElementById('momModal');
    ...
    // 4. EVENTOS PARA BOTONES (Incluso si no es la fecha, ella puede pulsarlos)
    const btnBday = document.getElementById('btnBirthday');
    const btnMom = document.getElementById('btnMomDay');
    ...
    // 5. MANTENER FUNCIONES ORIGINALES
